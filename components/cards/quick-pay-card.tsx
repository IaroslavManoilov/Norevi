...
export function QuickPayCard({
  ...
  useEffect(() => {
    ...
    const raw = readStorageWithLegacy(STORAGE_KEYS.paidToday(todayKey), `norevi_paid_today_${todayKey}`);
    if (!raw) return;
    try {
  ...
  useEffect(() => {
    if (typeof window === "undefined") return;
    const raw = readStorageWithLegacy(STORAGE_KEYS.paidMethod(todayKey), `norevi_paid_method_${todayKey}`);
    if (!raw) return;
    try {
  ...
  useEffect(() => {
    ...
    criticalBills.forEach((bill) => {
      const key = STORAGE_KEYS.criticalReminder(bill.id);
      const stored = readStorageWithLegacy(key, `norevi_critical_reminder_${bill.id}`);
  ...
  const payBill = async (bill: Bill, method: string) => {
    ...
    try {
      ...
      if (typeof window !== "undefined") {
        ...
        localStorage.setItem(STORAGE_KEYS.paidToday(todayKey), JSON.stringify(Array.from(nextSet)));
        const nextMethod = { ...paidMethod, [bill.id]: method };
        setPaidMethod(nextMethod);
        localStorage.setItem(STORAGE_KEYS.paidMethod(todayKey), JSON.stringify(nextMethod));
      }
