...
export function FinanceCalculatorCard({
  ...
  useEffect(() => {
    const key = STORAGE_KEYS.ratesSyncAt(baseCurrency);
    const last = Number(readStorageWithLegacy(key, `norevi_rates_sync_at_${baseCurrency}`) || 0);
    const shouldSync = Date.now() - last > 6 * 60 * 60 * 1000;
    if (!shouldSync) return;
    queueMicrotask(() => setRatesLoading(true));
      ...
      {showHeader ? (
        <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center">
          <FlowHeader
            ...
            title={t.title}
            className="mb-0"
            iconClassName="bg-[var(--brand-100)]"
