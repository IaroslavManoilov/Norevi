...
export function FoodPlanCard({
  ...
  const spendPlanStorageKey = STORAGE_KEYS.foodSpendPlan(monthKey);
  ...
  useEffect(() => {
    ...
    const saved = readStorageWithLegacy(STORAGE_KEYS.foodManualItems, "norevi_food_manual_items");
    if (saved) {
      try {
  ...
  useEffect(() => {
    if (typeof window === "undefined") return;
    localStorage.setItem(STORAGE_KEYS.foodManualItems, JSON.stringify(manualItems));
  }, [manualItems]);

  useEffect(() => {
    if (typeof window === "undefined") return;
    const saved = readStorageWithLegacy(spendPlanStorageKey, `norevi_food_spend_plan_${monthKey}`);
    if (saved) {
      try {
  ...
  useEffect(() => {
    ...
    const key = STORAGE_KEYS.weeklyStaples(weekStartKey);
    if (readStorageWithLegacy(key, `norevi_weekly_staples_${weekStartKey}`)) return;
