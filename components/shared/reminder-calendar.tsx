...
export function ReminderCalendar({
  ...
  useEffect(() => {
    ...
    todays
      ...
      .forEach((item) => {
        const storageKey = STORAGE_KEYS.reminderToast(item.id);
        if (readStorageWithLegacy(storageKey, `norevi_reminder_toast_${item.id}`)) return;
      ...
      {showHeader ? (
        <div className="mb-4 flex items-start justify-between gap-3">
          <FlowHeader
            ...
            hint={showHint ? t.calendarPlanner.hint : undefined}
            className="mb-0"
            iconClassName="h-8 w-8 bg-[var(--brand-100)]"
            labelClassName="text-xs"
            titleClassName="text-xl text-[var(--text)]"
          />
