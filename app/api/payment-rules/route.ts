...
import { listPaymentRules, upsertPaymentRule } from "@/features/payments/server/payment-rules-service";
...
export async function GET() {
  ...
  const { data, error } = await listPaymentRules(auth.supabase, auth.userId);
...
export async function POST(request: Request) {
  ...
  const { data, error } = await upsertPaymentRule(auth.supabase, auth.userId, payload.data);
